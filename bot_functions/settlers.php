...
 ⇒ [player]{$settlers[$v][0]}[/player], ♔ Baron Sent on the: [i]{$settlers[$v][1]}[/i]";
                  ...
                  if (is_array($settlers[$v])) {
                    ...
                    $lawless[$k] = '☑ ' . $lawless[$k] . "
 ⇒ [player]{$settlers[$v][0]}[/player], ♔ Baron ran since: [i]{$settlers[$v][1]}[/i]";
                    } else {
                    $lawless[$k] = '☒ ' . $lawless[$k] . "
...
", $settle);
              ...
              // post txt
              $post_update = "[u]Last Update[/u]: [i]" . date('d.m.Y H:i:s', $str_time) . "[/i] | [u]DataBase[/u]: [i]" . date('d.m.Y H:i:s', $last_update) . "[/i]" . (($update) ? ' | ('.count($new_residents).'|'.count($new_lawless).'|'.count($new_settler).')':'');
        ...
        case 'del':
          ...
          if ($data['command'][0] == PRE && Lou::is_string_pos($second_argument)) {
            ...
            if (!$settler || $settler != $data['user']) {
              if ($data["channel"] == ALLYIN) $message = "You have no {$lawless}claims on {$continent_name} [coords]{$pos}[/coords]";
              ...
              if ($data["channel"] == ALLYIN) $message = "Your {$lawless}claim on {$continent_name} [coords]{$pos}[/coords] has been deleted!";
              ...
              $bot->igm->send(implode(';',$receivers), "♚ C{$continent} {$pos} {$lawless}Deleted!", "[player]{$data['user']}[/player] as deleted the {$lawless}claim on Continent: {$continent} [coords]{$pos}[/coords]");
            }
          // error !
          } else {
            $message = 'Settler Error: Wrong Parameter!';
          }
          break;
      }
    // is position?
      ...
      if ($data['command'][0] == PRE) {
        ...
        if ($redis->SETNX("{$settler_key}:{$alliance_key}:{$continent_key}:settlers:{$pos}", $data['user'])) {
          ...
          $bot->log('IGM: send '.count($receivers).' messages to '.LoU::get_continent_abbr().$continent);
          $bot->igm->send(implode(';',$receivers), "♔ C{$continent} {$pos} {$lawless}Settle", "[player]{$data['user']}[/player] has made a {$lawless}claim on Continent: {$continent} [coords]{$pos}[/coords]");
        } else {
          $settler = $redis->get("{$settler_key}:{$alliance_key}:{$continent_key}:settlers:{$pos}");
          $lawless = ($redis->sIsMember("{$continent_key}:lawless", $pos)) ? 'LL ' : '';
          ...
          if ($settler != $data['user']) $message = "[player]{$settler}[/player] made a {$lawless}claim on {$continent_name} [coords]{$pos}[/coords] since: {$settletime}";
            ...
            if ($data["channel"] == ALLYIN) $message = "You already made a {$lawless}claim on {$continent_name} [coords]{$pos}[/coords] Since: {$settletime}";
        ...
        if ($settler = $redis->get("{$settler_key}:{$alliance_key}:{$continent_key}:settlers:{$pos}")) {
          $lawless = ($redis->sIsMember("{$continent_key}:lawless", $pos)) ? 'LL ' : '';
          $settletime = date('d.m.Y H:i:s', time() - (SETTLERTTL - $redis->TTL("{$settler_key}:{$alliance_key}:{$continent_key}:settlers:{$pos}")));
          $message = "[player]{$settler}[/player] has claimed {$continent_name} {$lawless}[coords]{$pos}[/coords] since: {$settletime}";
        } else $message = "Nobody has claimed {$continent_name} [coords]{$pos}[/coords]!";
      }
    } else if ($data['command'][0] == PRE) $bot->add_privmsg('Error: Wrong Parameter ('.$first_argument.')!', $data['user']);
    else return;
