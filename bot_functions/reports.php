...
$_opponent_alliance = ($_header['opponent_alliance'] >= 1) ? '-[[alliance]' . $redis->hget("alliance:{$_header['opponent_alliance']}:data", 'name') . '[/alliance]]' : '';
$_forum_text = str_replace($_header['opponent_name'], "[player]{$_header['opponent_name']}[/player]" . $_opponent_alliance  , $_header['report_text']);
...
[u]{$_header['category']}[/u] [u][b][city]{$_header['pos']}[/city][/b][/u] ~ [i]{$_header['name']}[/i] - [player]{$_header['owner_name']}[/player]";
$daily[$_header['pos']]['entrys'][] = " {$_img} " . date('H:i:s', $_header['time']) . " ~ [i]{$_forum_text}[/i]
 ⇒ [report]{$_header['report_link']}[/report]";
}
// post txt
$post_daily_head = "[b][u]Present day reports on {$thread_name}[/u][/b]

";
$post_daily_footer = "

[u]Last Update[/u]: [i]" . date('d.m.Y H:i:s', $str_time) . "[/i] | [u]DataBase[/u]: [i]" . date('d.m.Y H:i:s', $last_update) . "[/i]" . (($update) ? ' | ('.count($new_reports).')':'');
$chunks = array();
$post_daily = array();
$dailys = array();
if (!empty($daily)) {
...
if (count($entrys) >= $max_reports) $dailys[] = PHP_EOL . "(max. {$max_reports} pro City)";
}
...
} else $post_daily[] = $post_daily_head . "[i]No Reports Avalible[/i]" . $post_daily_footer;
...
// post txt
$post_weekly_head = "[b][u]Reports for the last 7 days on {$thread_name}[/u][/b]
...
if (count($entrys) >= $max_reports) $weeklys[] = PHP_EOL . "(max. {$max_reports} pro City)";
}
...
} else $post_weekly[] = $post_weekly_head . "[i]No Reports Avalible[/i]" . $post_weekly_footer;
...
// post txt
$post_update = "[u]{$_GAMEDATA->translations['tnf:worldmap legend']}[/u]: ♗ - [i]{$_GAMEDATA->translations['tnf:assault']}[/i], ♘ - [i]{$_GAMEDATA->translations['tnf:scout']}[/i], ♖ - [i]{$_GAMEDATA->translations['tnf:support']}[/i], ♚ - [i]{$_GAMEDATA->translations['tnf:siege']}[/i], ♙ - [i]{$_GAMEDATA->translations['tnf:plunder']}[/i]";
...
else $bot->add_privmsg("Mistake with the deletes from: ".BOT_REPORTS_FORUM."", $data['user']);
}
...
$alliance_key = "alliance:{$bot->ally_id}";
$bot->debug(count($reports['data']) . " Report header for {$pos} processed");
