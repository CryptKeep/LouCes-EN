<?php   
  // include main files
  require_once($_SERVER['ENV']['LIB'].'/config.php');
  require_once($_SERVER['ENV']['LIB'].'/redis.php');
  require_once($_SERVER['ENV']['LIB'].'/lou.php');
  
  // Standard inclusions      
  require_once($_SERVER['ENV']['LIB'].'/charts/pChart/pData.class');   
  require_once($_SERVER['ENV']['LIB'].'/charts/pChart/pChart.class');   
  require_once($_SERVER['ENV']['LIB'].'/charts/pChart/pCache.class');

  // whitch world?
  $server_world = $redis->get('server:world');   //      
  //   $server_world = 'World 90 (Europe)';
  
  
  // todo: fetch service with proper data
  $worlds = array('WORLD 90' => array('name' => 'W90', 'start' => 0) );

     //if (!$server_world) die('No world selected!(' . __FILE__ . ":" . __LINE__ . ")");    //  if (!$server_world) die('No world selected!');
  ...
  $stats = array('hours'  => 'Hours',
                 //'weeks'  => 'Woche',
                 //'months' => 'Monat',
                 //'years'  => 'Jahr'
                );
  $weeks = array(date("n") => 'This week');
  $months = array(date("n") => 'This month');
  ...
  */
  ...
  if ($inactive == 0) $inactive_text = " - Active!";
  else if ($inactive >= 365 ) $inactive_text = " ... not calculated!";
  else $inactive_text = " - {$inactive} Days Inactive!";
  /*
  ...
  */
  ...
  if (is_array($cities)) foreach ($cities as $cid => $pos) {
    ...
    $mDataSet->SetSerieName("Points","points");
    // Cache definition 
    $mCache = new pCache('../charts/Cache/');   // ../charts/Cache/
    ...
    $cDataSet->SetSerieName("Points","points");
    $cDataSet->SetYAxisName("Points");  
    $cDataSet->SetAbsciseLabelSerie("time");
    //$cDataSet->SetXAxisFormat("date");
    // Cache definition 
    $cCache = new pCache('../charts/Cache/');  // ../charts/Cache/
    if (($use_cache && !$cCache->GetFromCache(md5($cities[$cid]['data']['pos']) ,$cDataSet->GetData(), $cFileName)) OR !$use_cache) {
      ...
      if (!empty($mowner)) foreach($mowner as $k => $v) {
        ...
        if ($_un) $cTest->setLabel($cDataSet->GetData(),$cDataSet->GetDataDescription(),"points",$k,'Acquisitions: '.$_un,239,233,195);
      }
      $cTest->clearShadow(); 
  ...
  if (($use_cache && !$Cache->GetFromCache(md5($user['name']) ,$DataSet->GetData(), $FileName)) OR !$use_cache) {
    ...
    if (!empty($ccities)) foreach($ccities as $k => $v) {
      $Test->setLabel($DataSet->GetData(),$DataSet->GetDataDescription(),"points",$k,"Cities: {$v}",239,233,195);
    }
    $Test->clearShadow();
    // Clear the scale  
    $Test->clearScale();  
 ...
 <div id="wrapper">
  ...
  <form style="margin-top: 1em;" method="get" name="form" id="target">
   ...
   <div style="margin-left: 5px; float: left;">
    <button type="submit" id="submitBtn">Show</button>
   </div>
   <div style="margin-left: 5px; margin-top: 2px; float: left;">
    <input type="checkbox" name="advanced" id="advanceBtn" value="true"<?=($advanced) ? ' checked="checked"' : ''?>/><label for="advanceBtn">Extended</label>
   </div>
   <div style="margin-left: 5px; margin-top: 2px; float: left;">
    <input type="checkbox" name="c" id="clearcacheBtn" value="0"/><label for="clearcacheBtn">Delete Cache</label>
   </div>
   ...
   <div id="tabs" style="margin-left: 5px; margin-top: 2px; width: 701px;<?=($advanced) ? '' : ' display: none;'?>">
    <ul>
      <li><a href="#tabs-cities">Citys</a></li>
      <li><a href="#tabs-castles">Castles</a></li>
      <li><a href="#tabs-palasts">Palaces</a></li>
    </ul>
    <div id="tabs-cities">
      <div id="accordion-cities" class="accordions">
        ...
        <div>
          ...
          <div>Lalala.<br/><img src='<?=$c['cgraph']?>' alt='<?=$c['cgraph']?>' border='0'/></div>
        </div>
        <?php } else { ?>
          <p>No Citys</p>
        <?php } ?>
  ...
  <?php } else { ?>
  <p>Players not known!</p>
  <?php } ?>
<?php /*?>  <div class="ui-widget" style="margin-left: 5px; width: 707px;">
